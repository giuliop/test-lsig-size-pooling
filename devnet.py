"""Helper functions for testing and development on a local network"""
...
devnet_dir = os.path.expanduser("/Users/giulio/dev/algorand/devnet/network/data")
...
def compile(teal):
    """compile a teal program and return the result"""
    return b64decode(algod.compile(teal)['result'])

def sign_and_send(txn: sdk.transaction.Transaction) -> dict:
...
def fund(address: str, amount: int) -> dict:
    """Fund an address with the given amount of algo"""
    sp = algod.suggested_params()
    txn = sdk.transaction.PaymentTxn(pk, sp, address, amount)
    return sign_and_send(txn)
